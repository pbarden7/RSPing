...
def worldLatency(servers):
    returnlist = []
    for url, worldnum in servers.items():
        strlatency = ''
        stdout = Popen('ping -n 1 ' + url, shell=True, stdout=PIPE).stdout
        output = str(stdout.read())
        if 'timed out' in output:
            returnlist.append(worldnum + ' timed out')
            continue
...
for x, y in output2.items():
    print(x + ': ' + str(y) + 'ms') cy += str(latency[0])
        if int(strlatency) < 30:
            returnlist.append(strlatency + 'ms, ' + worldnum)
    return returnlist
output = worldLatency(worlds)
output.sort()
for i in output:
    print(i)
